...
const MedicalStaffManagementPage: React.FC = () => {
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const response: PaginatedUsers = await getAllUsers(
        ...
        debouncedSearchTerm,
        ['Doctor', 'Pharmacist', 'Nurse', 'LabStaff'], // Pass multiple roles to the service
        true
      );
  ...
  const columns: GridColDef<User>[] = React.useMemo(() => [
    ...
    { field: 'roles', headerName: 'Role', width: 150, valueGetter: (_value, row) => row.roles.join(', ') },
    {
      field: 'actions',
      headerName: 'Actions',
      width: 250,
  ...
  ], [setUserToEdit, setOpenEditUser, handleToggleRestrict]);
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <CreateAccountForm
        ...
        allowedRoles={['Doctor', 'Pharmacist', 'Nurse', 'LabStaff']}
      />
...
export default withAuth(MedicalStaffManagementPage, ['RecordStaff', 'Admin']);

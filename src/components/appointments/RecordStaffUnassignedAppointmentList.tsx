...
const RecordStaffUnassignedAppointmentList = () => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      fetchedAppointments.forEach(app => {
        ...
        } else if (app.doctorId && app.doctorName) { // Fallback for doctorId and doctorName if assignedDoctorId is not present
          initialSelectedDoctors[app.id] = {
  ...
  const handleAssignDoctor = async (appointmentId: string) => {
    ...
    try {
      await assignDoctorToAppointment(Number(appointmentId), Number(doctor.user_id)); // Convert to string as expected by backend

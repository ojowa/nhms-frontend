  ...
  const fetchStaffData = useCallback(async () => {
    ...
    try {
      ...
      if (deptDetails?.headUserId) {
        ...
        hodUser = staffResponse.find(s => s.userId === deptDetails.headUserId) ||
                  allUsersResponse.users.find(u => u.userId === deptDetails.headUserId) ||
                  null;
      }
      setCurrentHeadOfDepartment(hodUser);
      setCurrentStaff(staffResponse);
      ...
      const filteredAvailableUsers = allUsersResponse.users.filter(
              ...
              {currentStaff.length === 0 ? (
                ...
                <List dense>
                  {currentStaff.map((staffUser) => (
                    <ListItem
                      key={staffUser.userDepartmentId || staffUser.userId || `staff-${staffUser.email}`}
                      secondaryAction={
                        <IconButton
                          edge="end"
                          aria-label="remove"

  ...
  const handleSubmit = async () => {
    ...
    try {
      await updateLabResultStatusAndReviewers(
        ...
        String(user.roles.includes(UserRole.LabStaff) && status === 'PENDING_APPROVAL' ? user.userId : labResult.reviewed_by_user_id || ''),
        String((user.roles.includes(UserRole.Doctor) || user.roles.includes(UserRole.Admin)) && status === 'APPROVED' ? user.userId : labResult.approved_by_user_id || ''),
        String(user.userId)
      );

...
export default function Sidebar() {
  ...
  const mainItems = React.useMemo(() => {
    ...
    if (userRoles.includes(UserRole.Doctor)) {
    ...
    if (userRoles.includes(UserRole.Officer)) {
    ...
    const showPatientRelatedItems =
      userRoles.includes(UserRole.Patient) ||
      userRoles.includes(UserRole.Officer) ||
      (user?.patientType === 'Family');
  ...
  const secondaryItems = React.useMemo(() => {
    const items = [];
    if (
      userRoles.includes(UserRole.RecordStaff) ||
      userRoles.includes(UserRole.Officer) ||
      userRoles.includes(UserRole.Patient) ||

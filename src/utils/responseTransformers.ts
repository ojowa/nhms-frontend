...
export const transformAppointment = (data: any): Appointment => {
  return {
    ...
    dateTime: (data.date_time || data.appointment_time || data.dateTime) ? new Date(data.date_time || data.appointment_time || data.dateTime) : null,
    endTime: data.end_time || data.endTime ? new Date(data.end_time || data.endTime) : null,
    status: data.status,
    reason: data.reason,
    notes: data.notes,
